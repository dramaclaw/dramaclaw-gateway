...
const (
	// BrainClawCapabilityHeader carries DramaClaw's per-turn, short-lived
	// grouping capability. It is inbound only.
	BrainClawCapabilityHeader = "X-DramaClaw-Control-Capability"
	// BrainClawControlContextHeader is minted by this gateway alone, after the
	// outbound body is final. An inbound one is always a forgery attempt.
	BrainClawControlContextHeader = "X-DramaClaw-Control-Context"

	brainclawCapabilityHeaderLower     = "x-dramaclaw-control-capability"
	brainclawControlContextHeaderLower = "x-dramaclaw-control-context"
)

// stripBrainClawIdentityHeaders removes both identity headers from an outbound
// request unconditionally.
//
// The passthrough skip list above only governs name/regex passthrough rules.
// Header Override writes headers with an explicit Header.Set, so a channel
// configured with either name would inject a static, body-unbound value that
// looks authentic downstream. Stripping here, after every override has been
// applied and before anything is signed or sent, is what makes "only this
// gateway mints a Control Context" true rather than merely intended.
func stripBrainClawIdentityHeaders(req *http.Request) {
	if req == nil {
		return
	}
	req.Header.Del(BrainClawCapabilityHeader)
	req.Header.Del(BrainClawControlContextHeader)
}
...
var passthroughSkipHeaderNamesLower = map[string]struct{}{
	...
	// BrainClaw identity headers are hop-scoped and must never reach a
	// provider. The capability is inbound-only: this gateway consumes it and
	// mints its own Control Context from it. A channel configured with the "*"
	// passthrough rule would otherwise forward both.
	brainclawCapabilityHeaderLower:     {},
	brainclawControlContextHeaderLower: {},
...
// signBrainClawControlContext mints the request-bound Control Context, when the
// channel opts in and a verified capability is present.
//
// Only the JSON relay path is signed. DoFormRequest is deliberately excluded:
// a multipart body is streamed and has no single settled byte sequence to bind,
// so signing it would produce an envelope that cannot be re-verified.
func signBrainClawControlContext(c *gin.Context, req *http.Request, info *common.RelayInfo, requestBody io.Reader) {
	if c == nil || req == nil || info == nil {
		return
	}
	if !info.ChannelSetting.BrainClawControlContextEnabled {
		return
	}
	body, err := replayableRequestBody(req, requestBody)
	if err != nil {
		return
	}
	brainclaw.SignOutboundRequest(c, req, true, req.Method, req.URL.Path, body, time.Now().Unix())
}

// replayableRequestBody reads the exact bytes that will be sent. GetBody is set
// by ApplyUpstreamBodyMetadata above, so this consumes a replay rather than the
// live stream — reading the live one would leave the request empty.
func replayableRequestBody(req *http.Request, fallback io.Reader) ([]byte, error) {
	if req.GetBody == nil {
...
func DoApiRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*http.Response, error) {
	...
	// Last write wins, so this must follow every override. Stripping here is
	// what makes "only this gateway mints a Control Context" true rather than
	// merely intended: Header Override writes with an explicit Set and would
	// otherwise inject a static, body-unbound value.
	stripBrainClawIdentityHeaders(req)
	// By this point method, URL, body and headers are all final, which is
	// exactly what the signature has to bind. Signing is best-effort: a
	// failure leaves the request untouched and merely unattested, because
	// evidence collection must never be able to fail a user's request.
	signBrainClawControlContext(c, req, info, requestBody)
...
func DoFormRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*http.Response, error) {
	...
	applyHeaderOverrideToRequest(req, headerOverride)
	// Not a signing path — multipart bodies are out of scope for the Control
	// Context — but the identity headers must still never reach a provider.
	stripBrainClawIdentityHeaders(req)
	resp, err := doRequest(c, req, info)
	if err != nil {
		return nil, fmt.Errorf("do request failed: %w", err)
